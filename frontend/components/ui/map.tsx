...
// Vendored third-party map wrapper (mapcn). Its deliberate ref-during-render and
// effect-setState patterns trip the React Compiler-era lint rules; disabling them
// for this file only.
...
import { MAP_FLY_DURATION_MS } from "@/components/map/map-config"
...
// maplibre v6 ships ESM-only: inside the bundler module graph `import.meta.url`
// does not resolve to the worker file, so `defaultWorkerUrl()` yields an empty
// URL and the tile pipeline stalls forever (no vector tiles, no `load` event).
// Self-host the worker next to its sibling `maplibre-gl-shared.mjs` under
// `public/vendor/maplibre/` (the worker imports it via a relative path).
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  const [attributionButton, setAttributionButton] =
    ...
    <MapContext.Provider value={contextValue}>
      <div className={cn("map-shell relative h-full w-full", className)}>
        ...
        {attributionButton
          ? createPortal(
              <Info
                className="pointer-events-none size-4 lg:size-3.5"
                aria-hidden
              />,
              attributionButton
            )
...
function DefaultMarkerIcon() {
  return (
    <div className="relative h-4 w-4 rounded-full border-2 border-background bg-primary shadow-sm" />
  )
}
...
function PopupCloseButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      ...
      className="absolute top-1 right-1 z-10 inline-flex size-5 cursor-pointer items-center justify-center rounded-none text-foreground transition-colors hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset"
    >
...
type MapControlsProps = {
  ...
  /** Callback after the location fly animation finishes */
  onLocateEnd?: (coords: { longitude: number; latitude: number }) => void
  /** Extra control groups stacked with zoom / compass / locate */
  children?: ReactNode
}

const POSITION_INSET_PX = 16

const positionStyles: Record<
  NonNullable<MapControlsProps["position"]>,
  CSSProperties
> = {
  "top-left": { top: POSITION_INSET_PX, left: POSITION_INSET_PX },
  "top-right": { top: POSITION_INSET_PX, right: POSITION_INSET_PX },
  "bottom-left": { bottom: POSITION_INSET_PX, left: POSITION_INSET_PX },
  "bottom-right": { bottom: POSITION_INSET_PX, right: POSITION_INSET_PX },
}
...
function ControlGroup({ children }: { children: ReactNode }) {
  return (
    <div
      className={cn(
        "flex w-fit flex-col overflow-hidden rounded-none border border-border bg-background/80 shadow-sm backdrop-blur-xl supports-[backdrop-filter]:bg-background/70 [&>button:not(:last-child)]:border-b [&>button:not(:last-child)]:border-border",
        SURFACE_MICA_INTERACTIVE_CLASS
      )}
    >
      {children}
...
function ControlButton({
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "flex size-11 items-center justify-center rounded-none text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none focus-visible:ring-inset disabled:pointer-events-none disabled:opacity-50 lg:size-8"
      )}
      disabled={disabled}
    >
      {children}
...
function MapControls({
  ...
  const locateMoveEndRef = useRef<(() => void) | null>(null)

  useEffect(() => {
    return () => {
      if (map && locateMoveEndRef.current) {
        map.off("moveend", locateMoveEndRef.current)
        locateMoveEndRef.current = null
      }
    }
  }, [map])
        ...
        if (map) {
          ...
          if (onLocateEnd) {
            const handleMoveEnd = () => {
              ...
              if (locateMoveEndRef.current === handleMoveEnd) {
                locateMoveEndRef.current = null
              }
              onLocateEnd(coords)
            }
            locateMoveEndRef.current = handleMoveEnd
  ...
  const controlPosition =
    position === "bottom-right"
      ? {
          ...
          bottom:
            "calc(58px + var(--map-bottom-drawer-offset, 0px) + var(--map-controls-attribution-gap, 0px))",
        }
      : positionStyles[position]
...
function CompassButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <ControlButton onClick={onClick} label="Reset bearing to north">
      ...
      >
        <path d="M12 2L16 12H12V2Z" className="fill-[var(--secondary-chile)]" />
        <path
          d="M12 2L8 12H12V2Z"
          className="fill-[var(--secondary-chile)]/60"
        />
