...
**Related:** [AGENTS.md](../AGENTS.md) (índice), [FRONTEND.md](./FRONTEND.md) (componentes + data plumbing del monitor), [DOC-MAINTENANCE.md](../../docs/DOC-MAINTENANCE.md), `app/globals.css`, `lib/surface.ts`, `lib/risk-scale.ts`, `lib/alerts-display.ts`.

**División de verdad:** DESIGN = tokens, superficies, chrome visual. FRONTEND = capas MapLibre, hooks TQ, contrato de datos.
...
| Theme-aware surfaces + Mica | Dark-only `bg-black/60` glass from `old_frontend` |
...
- **Landing `/`:** full-bleed rotating globe + split wordmark Chile/Risk + one CTA (`/iniciar-sesion`). No citizen navbar, no Mica. Expressive shadows OK.
...
- **Citizen non-map (`/desastres`, stubs):** content under fixed top navbar (`pt-12` / `CITIZEN_NAVBAR_PAD_TOP_CLASS`); semantic page shell (`h-full overflow-y-auto` main). `/desastres` ahora tiene hero full-bleed de marca bajo navbar (`catalog/hero.png` + scrim institucional + stats mono); detalle de guía: hero imagen alto + columna de lectura `max-w-3xl`. `GlobePageBackground` exists but is **not mounted** yet — do not assume globe behind pages.
...
- **Alertas + Fecha:** desktop `lg+` → columna fija izquierda 320px (`SURFACE_PANEL_SHELL_CLASS`); `md`–`lg` → rail angosto flush left (`MAP_PANEL_RAIL_WIDTH_CLASS`) que se expande a 320px; móvil `<md` → FABs bottom-left + bottom `Sheet`. Prefs en `stores/ui-store` + persist `chilerisk-ui-v1`. Leyenda / Controles siguen TBD.
...
- Copy `glass-mica` / `GLASS_PANEL_*` from `old_frontend`
- `framer-motion` / unsolicited `sonner`

---

*Last updated: 2026-08-05 (desastres: hero full-bleed `catalog/hero.png` + scrim institucional + stats mono; headers de sección ops con counts)*
